...
import static com.funtl.my.shop.web.ui.constant.SystemConstants.SESSION_USER_KEY;
...
public class LoginController {
    ...
    public  String login1(TbUser tbUser,Model model,HttpServletRequest request) throws Exception {
        if (!checkVerification1(tbUser, request)){
    ...
    private  boolean checkVerification1(TbUser tbUser,HttpServletRequest request){
        String verification= (String) request.getSession().getAttribute(Constants.KAPTCHA_SESSION_KEY);
        if(StringUtils.equals(verification, tbUser.getVerification())){
            return  true;
        }
